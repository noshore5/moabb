...
if CLASSIFIER_TYPE == "wavelet":
    ...
    raise ValueError(
        f"Unknown classifier type: {CLASSIFIER_TYPE}. Use 'wavelet', 'cnn', 'cwtcnn', or 'eegnet'"
    )
...
def main():
    ...
    print("   Data will be used as-is: (n_samples, n_channels, n_timepoints)")
    ...
    if "session" in metadata.columns:
        ...
        train_mask = (
            (sessions == "T")
            | (sessions == "0train")
            | sessions_series.str.contains("train", case=False, na=False).values
        )
        eval_mask = (
            (sessions == "E")
            | (sessions == "1test")
            | sessions_series.str.contains("test", case=False, na=False).values
        )
    ...
    if CLASSIFIER_TYPE == "wavelet":
        clf = WaveletTransformClassifier(
            lowest=4, highest=40, nfreqs=50, sampling_rate=250
        )
        print("   Classifier: WaveletTransformClassifier")
        print("   Features: Wavelet transform + coherence -> Random Forest")
    elif CLASSIFIER_TYPE == "cnn":
        ...
        print("   Classifier: CoherenceCNNClassifier")
        print("   Features: Coherence matrices -> PyTorch CNN")
    elif CLASSIFIER_TYPE == "cwtcnn":
        ...
        print("   Classifier: CWTCNNClassifier")
        print("   Features: Raw wavelet transforms -> PyTorch CNN")
    elif CLASSIFIER_TYPE == "eegnet":
        ...
        print("   Classifier: EEGNetClassifier")
        print("   Features: Raw EEG signals -> EEG-specific CNN (~4k parameters)")

    print(
        f"   Frequency range: {clf.lowest}-{clf.highest} Hz"
        if hasattr(clf, "lowest")
        else "   (No frequency filtering - raw signal)"
    )
    ...
    try:
        coeffs, freqs = transform(
            debug_signal, frame_rate=250, highest=40, lowest=4, nfreqs=50
        )
        print("\n   ✓ Transform succeeded!")
        ...
        print(
            f"   Coefficients min/max: {np.abs(coeffs).min():.6f} / {np.abs(coeffs).max():.6f}"
        )
