...
class WaveletTransformClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _compute_wavelet_features(self, X):
        ...
        log.info(f"Computing wavelet features for {n_samples} samples, {n_channels} channels")
        log.info(f"Individual transforms: {individual_transforms}, Pairwise coherences: {pairwise_coherences}")
        log.info(f"Total operations: {total_operations}")
        print(f"   Starting {individual_transforms} wavelet transforms and {pairwise_coherences} coherence computations...", flush=True)
            ...
            for ch_idx in range(n_channels):
                ...
                if coeffs is not None:
                    ...
                    if abs_coeffs.ndim == 2:
                        ...
                        sample_features.extend(np.mean(abs_coeffs, axis=1))  # mean per scale
                        sample_features.extend(np.max(abs_coeffs, axis=1))   # max per scale
                        sample_features.extend(np.std(abs_coeffs, axis=1))   # std per scale
                    ...
                    if coeffs_i is not None and coeffs_j is not None:
                        try:
                            ...
                            if coh.ndim == 2:
                                ...
                                sample_features.extend(np.mean(coh, axis=1))  # mean coherence per scale
                                sample_features.extend(np.max(coh, axis=1))   # max coherence per scale
                                sample_features.extend(np.std(coh, axis=1))   # std coherence per scale
                            elif coh.ndim == 1:
                                # 1D coherence
        ...
        print(f"   ✓ Completed all wavelet transforms and coherence computations!", flush=True)
    ...
    def fit(self, X, y):
        ...
        log.info(f"Computing wavelet features for training data...")
