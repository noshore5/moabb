...
class CoherenceCNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _compute_coherence_matrices(self, X):
        ...
        log.info(
            f"Computing coherence matrices for {n_samples} samples, {n_channels} channels"
        )
        log.info(f"Number of unique channel pairs: {unique_pairs}")
        print(
            f"   Computing coherence matrices for {unique_pairs} channel pairs...",
            flush=True,
        )
        ...
        print(
            f"   ✓ Completed all {n_samples * n_channels} wavelet transforms!", flush=True
        )

        # Compute coherence matrices
        print(
            f"   Computing coherence for {n_samples * unique_pairs} pairs...", flush=True
        )
        coherence_matrices = []
        pair_count = 0
            ...
            for ch_i in range(n_channels):
                for ch_j in range(ch_i + 1, n_channels):
                    ...
                    if pair_count % max(1, total_pairs // 10) == 0:
                        print(
                            f"     Progress: {pair_count}/{total_pairs} pairs computed ({100*pair_count/total_pairs:.0f}%)",
                            flush=True,
                        )
        ...
        print(
            f"   Converting {len(coherence_matrices)} samples with coherence data to numpy array...",
            flush=True,
        )
        log.info(f"Converting {len(coherence_matrices)} samples to numpy array...")
        coherence_matrices = np.array(coherence_matrices)
        print(
            f"   ✓ Conversion complete! Array shape: {coherence_matrices.shape}",
            flush=True,
        )

        # Handle any NaN values
        print("   Cleaning NaN values...", flush=True)
        coherence_matrices = np.nan_to_num(
            coherence_matrices, nan=0.0, posinf=0.0, neginf=0.0
        )
        print("   ✓ Cleaned!", flush=True)

        log.info(f"Computed coherence matrices shape: {coherence_matrices.shape}")
        print("   ✓ Completed coherence matrix computation!", flush=True)
    ...
    def fit(self, X, y):
        ...
        if coherence_max > coherence_min:
            coherence_matrices = (coherence_matrices - coherence_min) / (
                coherence_max - coherence_min
            )
        else:
        ...
        log.info(
            f"Building CNN model with input shape: {(coherence_matrices.shape[1], coherence_matrices.shape[2], coherence_matrices.shape[3])}"
        )
        self.model_ = self._build_cnn_model(
            (
                coherence_matrices.shape[1],
                coherence_matrices.shape[2],
                coherence_matrices.shape[3],
            ),
            n_classes,
        )
    ...
    def predict(self, X):
        ...
        if self.coherence_max_ > self.coherence_min_:
            coherence_matrices = (coherence_matrices - self.coherence_min_) / (
                self.coherence_max_ - self.coherence_min_
            )
        else:
            coherence_matrices = np.zeros_like(coherence_matrices)
