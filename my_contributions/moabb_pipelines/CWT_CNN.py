...
class CWTCNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _compute_wavelet_transforms(self, X):
        ...
        log.info(
            f"Computing wavelet transforms for {n_samples} samples, {n_channels} channels"
        )
        ...
        for sample_idx in range(n_samples):
            ...
            if (sample_idx + 1) % max(1, n_samples // 10) == 0:
                print(
                    f"     Progress: {sample_idx + 1}/{n_samples} samples processed ({100*(sample_idx+1)/n_samples:.0f}%)",
                    flush=True,
                )

        # Convert to numpy array: (n_samples, n_channels, nfreqs, n_timepoints)
        print(
            f"   Converting {len(wavelet_transforms)} samples to numpy array...",
            flush=True,
        )
        log.info(f"Converting {len(wavelet_transforms)} samples to numpy array...")
        wavelet_transforms = np.array(wavelet_transforms)
        print(
            f"   ✓ Conversion complete! Array shape: {wavelet_transforms.shape}",
            flush=True,
        )

        # Handle any NaN values
        print("   Cleaning NaN values...", flush=True)
        wavelet_transforms = np.nan_to_num(
            wavelet_transforms, nan=0.0, posinf=0.0, neginf=0.0
        )
        print("   ✓ Cleaned!", flush=True)

        log.info(f"Computed wavelet transforms shape: {wavelet_transforms.shape}")
        print("   ✓ Completed wavelet transform computation!", flush=True)
    ...
    def fit(self, X, y):
        ...
        if transform_max > transform_min:
            wavelet_transforms = (wavelet_transforms - transform_min) / (
                transform_max - transform_min
            )
        else:
        ...
        log.info(
            f"Building CNN model with input shape: {(wavelet_transforms.shape[1], wavelet_transforms.shape[2], wavelet_transforms.shape[3])}"
        )
        self.model_ = self._build_cnn_model(
            (
                wavelet_transforms.shape[1],
                wavelet_transforms.shape[2],
                wavelet_transforms.shape[3],
            ),
            n_classes,
        )
    ...
    def predict(self, X):
        ...
        if self.transform_max_ > self.transform_min_:
            wavelet_transforms = (wavelet_transforms - self.transform_min_) / (
                self.transform_max_ - self.transform_min_
            )
        else:
            wavelet_transforms = np.zeros_like(wavelet_transforms)
